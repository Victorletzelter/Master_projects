...
def blindspot_pipeline(noisy_in,
    ...
    assert noise_style in ['gauss', 'poisson', 'impulse','brownian']
    ...
    if noise_style == 'gauss':
        ...
        if noise_params == 'known':
            ...
            noise_std = noise_est_out
    elif noise_style == 'brownian':
        if noise_params == 'known':
            noise_std = noise_params_in #The sigma value
        else:
            noise_std = noise_est_out
...
def noisify(x, style):
    ...
    elif style.startswith('brownian'): # Brownian noise
        def brown_noise(dims,sigma) :
            """
            This functions returns a brownian noise (also called random walk noise or red noise) and is adapted to the tensorflow framework
            with a dynamic size.
            Inputs: 
                - dims: 4D tensor.
                - sigma: Std of the desired brownian noise 
            Outputs :
                - B: 4D tensor containing the brownian noise.
            Remarks:
                - Constrains : dim[0]<=12, dim[1]<=3, dim[2]<=1000, dim[3]<=1000 ; the tf.slice trick was used to adapt to tensor objects.
            """ 
            listMbis=[]
            for j in range (12) :
                listM=[]
                for i in range (3) :
                    wn=np.random.randn(1000,1000)
                    ftwn=np.fft.fft2(wn)
                    n,p=np.shape(ftwn)
                    freq=np.zeros((n,p))
                    for i in range (n) :
                        for j in range (p) :
                            freq[i,j]=1/((i+1)**2+(j+1)**2)
                    res=ftwn*freq
                    M=np.fft.ifft2(res)
                    a,b=np.shape(M)
                    Mreal=np.zeros((a,b))
                    ...
                    Mreal=Mreal-np.mean(Mreal)
                    Mreal=sigma*(Mreal/np.std(Mreal))
                    #Mreal=(Mreal-np.min(Mreal))/(np.max(Mreal)-np.min(Mreal))
                    listM.append(Mreal)
                listMbis.append(np.stack(listM))
            B=np.stack(listMbis)
            B=tf.slice(tf.convert_to_tensor(B,dtype=tf.float32),[0,0,0,0],[dims[0],dims[1],dims[2],dims[3]])
            return(B)

        msh = tf.shape(x[:,:,:,:])
        M=brown_noise(msh,sigma=50/255)
        y=x+M
...
def train(submit_config,
    ...
    with tf.device("/gpu:0"):
        if eval_mode:
            ...
            if noise_style.startswith('brownian'): net_noise_style = 'brownian'
...
config_lst = [
    ...
    dict(eval_id = '00194', noise_style='brownian_25', num_iter=2000, pipeline='n2c')
]
